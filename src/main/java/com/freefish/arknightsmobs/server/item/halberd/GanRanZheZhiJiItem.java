...
public class GanRanZheZhiJiItem extends Item implements IVanishable, IAnimatable {
    public AnimationFactory factory = GeckoLibUtil.createFactory(this);
    private final Multimap<Attribute, AttributeModifier> tridentAttributes;

    public GanRanZheZhiJiItem(Properties builderIn) {
        super(builderIn);
        ImmutableMultimap.Builder<Attribute, AttributeModifier> builder = ImmutableMultimap.builder();
        builder.put(Attributes.ATTACK_DAMAGE, new AttributeModifier(ATTACK_DAMAGE_MODIFIER, "Tool modifier", 15.0D, AttributeModifier.Operation.ADDITION));
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, LivingEntity entityLiving, int timeLeft) {
        if (entityLiving instanceof PlayerEntity) {
            ...
            if (i >= 10) {
                if(entityLiving.rotationPitch<-55)
                    worldIn.addEntity(new SOTFEntity(worldIn,entityLiving,stack));
                else {
                    ...
                    if (j <= 0 || playerentity.isWet()) {
                        if (!worldIn.isRemote) {
                            ...
                            if (j == 0) {
                                GanRanZheZhiJi tridententity = new GanRanZheZhiJi(worldIn, playerentity, stack,false);
